...
#Creacion del Login de Usuario
    def loguin_usuario(self,correo_electronico,contraseña):
        try:
            correo_electronico=correo_electronico.upper()
            ...
            if isinstance(correo_electronico, str) and contraseña != None: 
                ...
                if usuarioL[0][0] == correo_electronico and contraseñaL[0][0] == str(contraseña):
                    resultado1 = conectar_mysql(Orden ="Select id_persona_cuit from inversores where correo_electronico = ('{0}')".format(correo_electronico))
                    print(int(resultado1[0][0]))
                    resultado1=resultado1[0][0]
                    usuario_id = conectar_mysql(Orden ="SELECT COUNT(id_persona_cuit) from billeteras where id_persona_cuit = ('{0}')".format(resultado1))
                    print(usuario_id[0][0])
                    if usuario_id[0][0]==0:
                        conectar_mysql(Orden="INSERT INTO billeteras (id_persona_cuit) VALUES (%s)",Valores=[resultado1])
                        print(correo_electronico + ",    Bienvenido a ARG BROKER DEMO")
                    else:
                        print(correo_electronico + ",    Bienvenido nuevamente a ARG BROKER DEMO") 
                else:
    ...
    def alta_inversor(self):
        try:
            ...
            # Si no existe, proceder a la inserción
                ...
                id_localidad=id_localidad.upper()
                ...
                valores = (id_persona_cuit, nombre, id_localidad, ex_politica, calle, numero_calle, correo_electronico.upper(), contraseña)
                conectar_mysql(orden, valores)
                print("Usuario creado con exito, volver al cargar el sistema.")
        except Exception as e:
...
#Getters y Setters
    ...
    def traer_id_inversor(self,correo_electronico):
                id_usuario=conectar_mysql(Orden="SELECT id_persona_cuit FROM inversores WHERE correo_electronico=('{0}')".format(correo_electronico))
